class GeometryOps {
 ...
 static normal(g,face){const f=g.faces[face],a=g.vertices[f[0]],b=g.vertices[f[1]],c=g.vertices[f[2]];const u=[b[0]-a[0],b[1]-a[1],b[2]-a[2]],v=[c[0]-a[0],c[1]-a[1],c[2]-a[2]];let n=[u[1]*v[2]-u[2]*v[1],u[2]*v[0]-u[0]*v[2],u[0]*v[1]-u[1]*v[0]];const l=Math.hypot(...n)||1;return n.map(x=>x/l)}
 static center(g,face){const f=g.faces[face],c=[0,0,0];f.forEach(i=>{c[0]+=g.vertices[i][0];c[1]+=g.vertices[i][1];c[2]+=g.vertices[i][2]});return c.map(x=>x/f.length)}
 ...
 static inset(g,face,amount){if(!g.faces[face])return g;const f=g.faces[face],c=this.center(g,face);const ids=f.map(i=>{const v=g.vertices[i];const q=[v[0]+(c[0]-v[0])*amount,v[1]+(c[1]-v[1])*amount,v[2]+(c[2]-v[2])*amount];g.vertices.push(q);return g.vertices.length-1});g.faces[face]=ids;for(let i=0;i<f.length;i++){const j=(i+1)%f.length;g.faces.push([f[i],f[j],ids[j],ids[i]])}return g}
 static bevel(g,face,amount){return this.inset(g,face,Math.min(.8,Math.max(.02,amount))) }
